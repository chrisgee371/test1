...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/test3_cg")
    spark.conf.set("spark.default.parallelism", "4")
    spark.conf.set("spark.sql.legacy.allowUntypedScalaUDF", "true")
    registerUDFs(spark)
    
    MetricsCollector.instrument(spark = spark, pipelineId = "pipelines/test3_cg", config = Config)(pipeline)
