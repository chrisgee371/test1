...
class Inc_Load_for_Inc(MetaGemExec):
    ...
    def execute(self, spark: SparkSession, subgraph_config: SubgraphConfig) -> List[DataFrame]:
        ...
        df_Inc_Directory_Listing = Inc_Directory_Listing(spark)
        df_Inc_Defensive_checks_UDF = Inc_Defensive_checks_UDF(spark, df_Inc_Directory_Listing)
        df_Inc_Read_Deduplicate_Write_Delta = Inc_Read_Deduplicate_Write_Delta(spark, df_Inc_Defensive_checks_UDF)
        df_Inc_RowDistributor_out0, df_Inc_RowDistributor_out1 = Inc_RowDistributor(
            spark, 
            df_Inc_Read_Deduplicate_Write_Delta
        )
        Incremental_load_operations(spark, df_Inc_RowDistributor_out0)
        inc_Duplicate_Record_Logger(spark, df_Inc_RowDistributor_out1)
